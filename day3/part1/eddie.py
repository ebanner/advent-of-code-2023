...
def explore_symbol(grid, i, j):
    ...
    """
    sum = 0
    sum +=explore_number(grid, i, j-1) # left
    sum += explore_number(grid, i-1, j-1) # upper left
    sum += explore_number(grid, i-1, j) # up
    sum += explore_number(grid, i-1, j+1) # upper right
    sum += explore_number(grid, i, j+1) # right
    sum += explore_number(grid, i+1, j+1) # lower right
    sum += explore_number(grid, i+1, j) # down
    sum += explore_number(grid, i+1, j-1) # lower right
    return sum
